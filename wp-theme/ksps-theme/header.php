...
<html <?php language_attributes(); ?>>
  ...
  <body <?php body_class('antialiased bg-white'); ?> data-assets-url="<?php echo esc_url(get_stylesheet_directory_uri() . '/assets'); ?>">
    ...
    <header class="bg-background fixed top-0 left-0 z-100 w-full lg:px-20 px-4 py-4 flex items-center justify-between shadow-[0_4px_6px_-1px_rgba(0,0,0,0.15)]">
      ...
      <nav class="lg:flex hidden items-center gap-6">
        <div id="menu-nav-desktop" class="flex items-center gap-2 relative cursor-pointer group">
          ...
          <div id="menu-dropdown-desktop" class="bg-background lg:absolute top-full left-0 h-fit w-88.75 p-6 flex flex-col gap-6 border border-border rounded-b-xl hidden">
            <a class="text-text-primary font-inter text-[16px] hover:text-secondary" href="#">Layanan IT Outsourching</a>
        ...
        <div class="flex items-center gap-2 cursor-pointer group">
          <label class="font-inter text-[16px] text-text-primary group-hover:text-secondary cursor-pointer">Referal</label>
        </div>
      </nav>
      ...
      <a id="sidebar-toggle" class="lg:hidden block" href="#">
        ...
        <svg id="sidebar-open-icon" xmlns="http://www.w3.org/2000/svg" width="24" height="16" viewBox="0 0 24 16" fill="none">
          <path d="M1 1H23M1 8H23M1 15H23" stroke="#FA9A07" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
        </svg>
      </a>
